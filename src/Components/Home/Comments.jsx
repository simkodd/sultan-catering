  ...
  return (
    <section className="relative py-20 min-h-210 flex flex-col justify-center items-center bg-transparent overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10 w-full max-w-6xl">
        ...
        {/* Flex yapısı ile başlık sola, butonlar sağa yaslanır */}
        <div className="mb-12 md:mb-20 flex flex-col md:flex-row md:items-end md:justify-between gap-8 max-w-2xl md:max-w-none mx-auto">
          ...
          <div className="text-center md:text-left">
            <span className="inline-block bg-bej/25 text-bej px-5 py-2 rounded-b-3xl text-xs mb-4 uppercase">
              Client Reviews
            </span>
            <h2 className="font-roboto font-medium text-3xl md:text-5xl text-bej leading-tight mb-4 drop-shadow-[0_1px_1px_#00000050] max-w-160">
              Discover What Our Clients Say About Our Catering.
            </h2>
          </div>

          {/* SAĞ: Masaüstü Butonları (Sadece Masaüstünde Görünür - hidden md:flex) */}
          <div className="hidden md:flex gap-4 mb-2">
            <button 
              onClick={handlePrev} 
              className="bg-bej text-red w-16 h-16 rounded-full flex items-center justify-center text-2xl shadow-lg hover:scale-110 active:scale-95 transition-transform"
            >
              <FaArrowLeft />
            </button>
            <button 
          ...
          {reviews.map((review, index) => {
            ...
            return (
              <motion.div
                ...
                initial={false}
                animate={position}
                className="absolute w-full max-w-88 md:max-w-140 bg-bej/90 backdrop-blur-md rounded-b-[50px] p-8 md:p-12 shadow-2xl top-0"
                style={{ height: "auto", minHeight: "400px" }}
              >
                {/* KİŞİ BİLGİSİ */}
                <div className="flex items-center gap-4 mb-6">
                  <div className={`w-14 h-14 md:w-16 md:h-16 ${review.bgColor} rounded-full flex items-center justify-center text-bej text-xl md:text-2xl font-bold shadow-md`}>
                    {review.initials}
                  </div>
                  <div>
                    ...
                    <p className={`text-xs md:text-sm font-medium opacity-70 ${review.themeColor}`}>
                      {review.role}
                    </p>
                  </div>
                </div>
                ...
                <div className="relative">
                  <FaQuoteLeft className={`absolute -top-3 -left-2 text-4xl opacity-10 ${review.themeColor}`} />
                  <p className={`relative z-10 text-base md:text-lg leading-relaxed font-medium ${review.themeColor} line-clamp-6 md:line-clamp-none`}>
                    {review.text}
                  </p>
                </div>
              </motion.div>
        ...
        {/* --- MOBİL BUTONLARI (Sadece Mobilde Görünür - flex md:hidden) --- */}
        {/* Kartların altında yer alır */}
        <div className="flex md:hidden items-center justify-center gap-6 ">
          <button 
            onClick={handlePrev} 
            className="bg-bej text-red w-14 h-14 rounded-full flex items-center justify-center text-xl shadow-lg active:scale-95 transition-transform"
          >
            <FaArrowLeft />
          </button>
